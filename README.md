...
「session」有两种：**交互式会话**（一个你能看到、能在窗口里继续操作的 Claude Code）和 **headless worker**（无窗口、后台、用 `claude -p` 跑、自己收发飞书）。下面 4 种方式覆盖桌前 / 远程、交互 / headless 各场景。

| 方式 | 类型 | 在哪触发 | 适用 |
|---|---|---|---|
| A. 开窗口 + `/waga-on` | 交互式 | 电脑前 | 最常规；出门前把当天要用的都开好 |
| B. 命令行起 worker | headless | 任意终端 | 不想占一个窗口、跑后台任务 |
| C. `!` 前缀起 worker | headless | Claude Code 输入框 | 已经在一个会话里，顺手起一个 |
| D. 飞书一句话让 Claude 起 | headless | **飞书（远程）** | 人在外面，唯一可行的远程起法 |
...
worker 后台常驻，监听 `[名字]` 的飞书消息，每条用 `claude -p --resume <固定session-id>` 处理（**保持上下文，跟正常会话一样**），结果发回飞书。

### 方式 C · `!` 前缀（已在一个 Claude Code 会话里时）

在 Claude Code 的输入框里直接敲（`!` 是输入框的本地执行前缀）：
...
### 方式 D · 飞书一句话，让 Claude 远程起（人在外面用这个）

对任意已挂载的会话，在飞书说一句自然语言，Claude 就会去跑 `waga-spawn.sh`：

```
waga: 起个新 session 叫 api，去 /path/to/proj 干「初始任务」
```

#### ⚠ 方式 D 的一次性前置：加权限规则（必须你本人在电脑前做一次）

headless worker 用到 `--dangerously-skip-permissions`（headless 弹不出权限确认框，否则卡住）。Claude Code 的安全策略**禁止 agent 自己授权**这种自主进程，也禁止 agent 改配置来给自己开——所以这条规则**只能你手动加**（加一次，永久生效）：

编辑 `~/.claude/settings.json`，在 `permissions.allow` 数组里加一条：

```json
"Bash(bash \"/path/to/waga-feishu-skill/waga-spawn.sh\"*)"
```

> 这是有意的安全边界：放权给「无人值守、自动跳过权限、执行 IM 指令」的 agent，必须真人点头。光有宽泛的 `"Bash"` 不够——分类器是意图层，要这条**精确规则**才放行。没加这条，远程 spawn 起不来。
>
> 因为加规则得在电脑前，所以方式 D 是「桌前一次性设置好、之后随时远程用」。
