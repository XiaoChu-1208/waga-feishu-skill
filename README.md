...
## 进阶：远程 spawn 新会话（headless worker）

前面说过 Waga 开不了终端、得提前开好窗口。`waga-spawn.sh` 是这条限制的**部分解法**：让你（在外面，通过飞书）让一个已挂载的会话**直接拉起一个无需窗口的新 Claude worker**。

它怎么工作：worker 是个后台进程，监听发给 `[<名字>]` 的飞书消息，每来一条就用 `claude -p --resume <固定session-id>` 跑一个 headless Claude 处理（保持上下文，跟正常会话一样），结果发回飞书。

### ⚠ 一次性前置：加权限规则（必须你本人在电脑前做）

spawn 一个 headless worker 会用到 `--dangerously-skip-permissions`（headless 弹不出权限框，否则卡住干不了活）。Claude Code 的安全策略**禁止 agent 自己授权**这种自主进程——所以这条规则**只能你手动加**：

编辑 `~/.claude/settings.json`，在 `permissions.allow` 加一条：

```json
"Bash(bash \"/path/to/waga-feishu-skill/waga-spawn.sh\"*)"
```

> 这是有意的安全边界：放权给「无人值守、自动执行 IM 指令、跳过权限确认」的 agent，必须真人点头。没加这条，远程 spawn 起不来（你只能在电脑前用 `! bash waga-spawn.sh ...` 亲手起）。

### 触发 / 使用

加好规则后，在飞书对已挂载的会话说一句（自然语言即可，Claude 会去跑 `waga-spawn.sh`）：
...
本地关闭：`touch /tmp/waga_stop_<名字>.txt`，或直接杀掉进程（`pkill -f "waga-spawn.sh <名字>"`）。关掉即注销，无残留。

### 直接命令（不经自然语言）

```bash
bash "$WAGA_DIR/waga-spawn.sh" <名字> "初始任务" "/工作目录"
```
