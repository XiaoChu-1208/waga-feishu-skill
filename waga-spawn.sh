...
# 本地配置：真实 chat_id / open_id 等放同目录 .env（已 gitignore，绝不上传）；
# 公开用户也可改用直接 export 环境变量，二选一即可。
...
# Python 启动器：Windows 用 `py`（Git Bash 里 `python` 是 WindowsApps 坏桩，exit 49 吞输出），
# Mac/Linux 没有 `py`，用 `python3`。按顺序探测，谁在用谁。
...
dispatch() {
  ...
  case "$body" in
    ...
    "/cd "*)
      ...
      [ -n "$mid" ] && react "$mid" DONE ;;
    "/who"|"/who "*)
      # /who 是全局名册指令（windowed worker 出名册卡）：headless worker 不当任务处理，
      # 只回个 Get 表情致意，避免被有粘性的 worker 误当任务跑。
      # 注意：只精确匹配 /who，不要用 "/"* 兜底——否则会误吞 /clear 等其它合法输入。
      # /stop /status /cd 已在上面各自处理，不受影响。
      [ -n "$mid" ] && react "$mid" Get ;;
    *)
      handle "$mid" "$body" ;;
  esac
...
while true; do
  ...
  while IFS=$'\t' read -r mid replyto content; do
    ...
    case "$content" in
      ...
      "${NAME}:"*|"${NAME}："*)
        # 冒号带内容 → 切粘性 + 处理（reseed 封档：本条之外的旧消息不补读）
